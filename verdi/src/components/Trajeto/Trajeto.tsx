...
export default function Trajeto(props: {id:number}) {
  ...
  const iniciarTrajeto = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(`https://python-verdi-deploy.vercel.app/registrar`, {
        method: "POST",
  ...
  return (
    <section className='flex flex-col lg:flex-row lg:justify-around lg:items-center w-screen'>
      <div className='md:w-screen lg:w-[40vw] flex flex-col items-center'>
        ...
        <form onSubmit={iniciarTrajeto} className='flex flex-col items-center gap-6 container w-11/12 md:w-1/2 lg:w-full bg-white p-5 sm:p-8 rounded-2xl'>
          <input
            type="text"
            placeholder="Início"
            value={origem}
      ...
      { mostrarMapa === true ? <div className='w-[90vw] h-[500px] lg:w-[40vw] mx-auto lg:m-0'>
