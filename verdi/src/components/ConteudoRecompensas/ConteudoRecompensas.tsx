  ...
  return (
    <section className="grid grid-cols-1 w-72 gap-3 sm:grid-cols-2 sm:w-11/12 md:grid-cols-3 lg:w-2/3">
        {ListaRecompensa.map((info) => (
            <div key={info.id} className="border-2 bg-emerald-950 border-green-700 p-2 flex flex-col justify-between h-72 relative">
              <Link href={`/rewards/${info.id}`} className="absolute w-full h-full"></Link>
                ...
                <div className="flex justify-between">
                    ...
                    <p className="text-green-400">{info.custo_pontos} pontos</p>
                </div>
            </div>
        ))}
    </section>
